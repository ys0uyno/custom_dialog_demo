...
HMODULE g_hook_mfc_dialog_hmodule;
                     ...
                     int       nCmdShow)
{
	...
	g_hook_mfc_dialog_hmodule = LoadLibrary(L"hook_mfc_dialog");
	if (NULL == g_hook_mfc_dialog_hmodule)
	{
		OutputDebugString(L"LoadLibrary hook_mfc_dialog failed");
	}

	g_beg_dialog_hook = (BEGDIALOGHOOK)GetProcAddress(g_hook_mfc_dialog_hmodule, "BegDialogHook");
	if (NULL == g_beg_dialog_hook)
	{
		OutputDebugString(L"g_beg_dialog_hook null");
	}

	g_end_dialog_hook = (ENDDIALOGHOOK)GetProcAddress(g_hook_mfc_dialog_hmodule, "EndDialogHook");
...
void free_all_dlls()
{
	if (g_hook_mfc_dialog_hmodule)
		FreeLibrary(g_hook_mfc_dialog_hmodule);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DIALOG_UI_BEG:
		_stprintf_s(sz, L"WM_DIALOG_UI_BEG wparam: %x", wParam);
